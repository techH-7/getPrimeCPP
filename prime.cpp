...
static void k_h();
static void k_i();
static void k_k(int64_t k_ae /*excpt*/);
static void k_n(type_(Array_<char16_t>) k_ah /*str*/);
template<typename T_>static type_(Array_<char16_t>) k_ad(T_ k_ai /*me_*/, type_(Array_<int64_t>) k_aj /*type*/);
static int64_t argc_;
static char** argv_;
static type_(Array_<char16_t>) k_l;
static char16_t k_m;
// k_main_
void k_main_(){
int64_t k_b; // $
...
// _initVars
static void k_e(){
(k_l) = (newArrayBin_<char16_t>(3,"\x20\x00\x2C\x00\x0A\x00"));
(k_m) = (u'\u0000');
}
// main
static void k_f(){
...
int64_t k_s;
int64_t k_y;
(k_n)((newArrayBin_<char16_t>(19,"\x31\x00\x30\x00\x30\x00\xE5\x4E\x0B\x4E\x6E\x30\x20\x7D\x70\x65\x92\x30\x42\x6C\x81\x30\x8B\x30\xD7\x30\xED\x30\xB0\x30\xE9\x30\xE0\x30\x3A\x00\x0A\x00")));
for(k_r = (2LL), k_s = (100LL); k_r <= k_s; k_r += (1LL)){
(k_t) = (false);
for(k_x = (2LL), k_y = ((k_r) - (1LL)); k_x <= k_y; k_x += (1LL)){
if(((k_r) % (k_x)) == (0LL)){
(k_t) = (true);
}
}
if(!(k_t)){
(k_n)((((newArrayBin_<char16_t>(0,""))->Cat(k_ad<int64_t>((k_r), (new_(Array_<int64_t>)((0LL))))))->Cat(newArrayBin_<char16_t>(1,"\x0A\x00"))));
}
}
}
// _finVars
...
#if defined(_WIN32)
fputws(reinterpret_cast<const wchar_t*>(k_ah->B),stdout);
#else
std::u16string s_=k_ah->B;
